...
export default class TableRenderer {
  ...
  drawDiscardArea(ctx, state, layout) {
    ...
    if (recent) {
      this.drawCard(ctx, recent.card, area.x + area.width / 2 - 22, area.y + 36, 44, 64, true);
      ctx.fillStyle = '#fff7dc';
      ctx.font = '13px Arial';
  ...
  drawPlayerHand(ctx, state, layout) {
    layout.handCards.forEach((region) => {
      ...
      this.drawCard(ctx, region.card, region.x, region.y, region.width, region.height, true, selected);
    });
  }
  ...
  drawCard(ctx, card, x, y, width, height, front = true, selected = false, size = 'vertical') {
    if (!front) {
  ...
  drawCardBack(ctx, x, y, width, height, size = 'vertical') {
