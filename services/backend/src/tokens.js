const crypto = require('crypto');

function base64UrlEncode(value) {
  return Buffer.from(value).toString('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=+$/g, '');
}

function base64UrlDecode(value) {
  const input = String(value || '').replace(/-/g, '+').replace(/_/g, '/');
  const padding = input.length % 4 ? '='.repeat(4 - (input.length % 4)) : '';
  return Buffer.from(input + padding, 'base64').toString('utf8');
}

function signPayload(encodedPayload, secret) {
  return base64UrlEncode(crypto.createHmac('sha256', secret).update(encodedPayload).digest());
}

function issueToken(openid, options = {}) {
  if (!openid) throw new Error('OPENID_REQUIRED');
  const now = options.now || Date.now();
  const ttlMs = options.ttlMs || 10 * 60 * 1000;
  const payload = {
    v: 1,
    typ: options.type || 'socket',
    openid,
    iat: now,
    exp: now + ttlMs,
    nonce: options.nonce || crypto.randomBytes(12).toString('hex'),
  };
  const encodedPayload = base64UrlEncode(JSON.stringify(payload));
  const signature = signPayload(encodedPayload, options.secret || '');
  return {
    token: `${encodedPayload}.${signature}`,
    expiresAt: payload.exp,
    issuedAt: payload.iat,
  };
}

function verifyToken(token, options = {}) {
  const parts = String(token || '').split('.');
  if (parts.length !== 2 || !parts[0] || !parts[1]) return { ok: false, error: 'TOKEN_MALFORMED' };
  const [encodedPayload, signature] = parts;
  const expected = signPayload(encodedPayload, options.secret || '');
  const actualBuffer = Buffer.from(signature);
  const expectedBuffer = Buffer.from(expected);
  if (actualBuffer.length !== expectedBuffer.length || !crypto.timingSafeEqual(actualBuffer, expectedBuffer)) {
    return { ok: false, error: 'TOKEN_SIGNATURE_INVALID' };
  }
  let payload;
  try {
    payload = JSON.parse(base64UrlDecode(encodedPayload));
  } catch (err) {
    return { ok: false, error: 'TOKEN_PAYLOAD_INVALID' };
  }
  const now = options.now || Date.now();
  if (!payload.openid) return { ok: false, error: 'TOKEN_OPENID_MISSING' };
  if (options.type && payload.typ !== options.type) return { ok: false, error: 'TOKEN_TYPE_MISMATCH' };
  if (typeof payload.exp !== 'number' || payload.exp <= now) return { ok: false, error: 'TOKEN_EXPIRED' };
  if (options.openid && payload.openid !== options.openid) return { ok: false, error: 'TOKEN_OPENID_MISMATCH' };
  return { ok: true, openid: payload.openid, payload };
}

function issueAppToken(openid, config, options = {}) {
  return issueToken(openid, {
    type: 'app',
    secret: config.appTokenSecret,
    ttlMs: options.ttlMs || config.appTokenTtlMs,
    now: options.now,
    nonce: options.nonce,
  });
}

function issueSocketToken(openid, config, options = {}) {
  return issueToken(openid, {
    type: 'socket',
    secret: config.socketTokenSecret,
    ttlMs: options.ttlMs || config.socketTokenTtlMs,
    now: options.now,
    nonce: options.nonce,
  });
}

function verifyAppToken(token, config, options = {}) {
  return verifyToken(token, Object.assign({}, options, { type: 'app', secret: config.appTokenSecret }));
}

function verifySocketToken(token, config, options = {}) {
  return verifyToken(token, Object.assign({}, options, { type: 'socket', secret: config.socketTokenSecret }));
}

module.exports = {
  issueAppToken,
  issueSocketToken,
  issueToken,
  verifyAppToken,
  verifySocketToken,
  verifyToken,
};
