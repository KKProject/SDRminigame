...
class HuapaiEngine {
  ...
  applyAction(action) {
    ...
    if (action.type === 'zhao') {
      meld.zhaoSize = action.zhaoSize || applied.cards.length;
  ...
  findPlayerAction(seatIndex, ref = {}) {
    ...
    return list.find((action) => {
      ...
      if (typeof ref.zhaoSize === 'number' && action.zhaoSize !== ref.zhaoSize) return false;
      if (typeof ref.handKeyCount === 'number' && action.handKeyCount !== ref.handKeyCount) return false;
