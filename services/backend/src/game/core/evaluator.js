...
function findChiActions(state, seatIndex, incomingCard, sourceSeat, sourceType, rules = DEFAULT_RULES) {
  ...
  if (hasManuallyDiscardedHandKey(seat, incomingCard.key)) {
    if (!specialTazi) return [];
    return [{
      type: 'circle-loss',
      seat: seatIndex,
      card: incomingCard,
      ...
      label: ACTION_LABELS.circleLoss || '进圈',
      forced: true,
      hidden: true,
      reason: `必须吃${incomingCard.text}，但已从手牌打过该字，进圈`,
      reasonCode: 'manual-hand-discard-blocked-forced-chi',
      forcedPattern: specialTazi.pattern,
    }];
  }
...
/**
 * 特殊搭子（塌子）检测：手牌恰 3 张同句，且这 3 张呈"2+1"分布（任意两字组合，
 * 覆盖全部 6 种排列，如 xxy/xxz/yyx/yyz/zzx/zzy）。incoming 为缺失字时吃/碰为
 * forced（必须操作，否则过则进圈）。
 */
