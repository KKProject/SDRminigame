...
export function runSelfChecks() {
  ...
  assert(legalDiscardKeys(xxxyzSeat) === 'da,ren', 'xxxyz should lock the completed xxx triplet and only allow discarding y and z');
  assert(!isLegalDiscard(xxxyzSeat, xxxyzSeat.hand.find((card) => card.key === 'shang'), DEFAULT_RULES).legal, 'xxxyz should reject discarding any card from the completed xxx triplet');
  discardFromSeat(xxxyzSeat, 'da');
  assert(legalDiscardKeys(xxxyzSeat) === 'ren', 'xxxyz should keep allowing the remaining singleton after discarding one extra');
  discardFromSeat(xxxyzSeat, 'ren');
  assert(getLegalDiscards(xxxyzSeat, DEFAULT_RULES).length === 0 && xxxyzSeat.hand.length === 3, 'xxxyz should lock the preserved xxx triplet once both extras are discarded');
  const xxyyzzSeat = createSeats(DEFAULT_RULES, 0)[0];
  xxyyzzSeat.hand = cardsFor(['shang', 'shang', 'da', 'da', 'ren', 'ren']);
  assert(getLegalDiscards(xxyyzzSeat, DEFAULT_RULES).length === 0, 'xxyyzz should be fully locked since it tiles exactly into two xyz doors');
  const xxxxSeat = createSeats(DEFAULT_RULES, 0)[0];
  xxxxSeat.hand = cardsFor(['shang', 'shang', 'shang', 'shang']);
  assert(getLegalDiscards(xxxxSeat, DEFAULT_RULES).length === 0, 'xxxx should be fully locked even though trimming down to a bare xxx would otherwise be reachable');
  ...
  assert(legalDiscardKeys(zzzxxySeat) === 'da,shang,shang', 'zzzxxy should lock the completed zzz triplet and only allow discarding x and y');
  assert(!isLegalDiscard(zzzxxySeat, zzzxxySeat.hand.find((card) => card.key === 'ren'), DEFAULT_RULES).legal, 'zzzxxy should reject discarding any card from the completed zzz triplet');
  discardFromSeat(zzzxxySeat, 'shang');
  assert(legalDiscardKeys(zzzxxySeat) === 'da,shang', 'zzzxxy should keep allowing the remaining x and y after discarding one extra');
  discardFromSeat(zzzxxySeat, 'shang');
  assert(legalDiscardKeys(zzzxxySeat) === 'da', 'zzzxxy should allow discarding down to the final non-triplet card');
  discardFromSeat(zzzxxySeat, 'da');
  assert(getLegalDiscards(zzzxxySeat, DEFAULT_RULES).length === 0 && zzzxxySeat.hand.length === 3, 'zzzxxy should lock the preserved zzz triplet once all extras are discarded');
