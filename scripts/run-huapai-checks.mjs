...
const files = ['rules', 'cards', 'evaluator', 'ai', 'engine', 'layout', 'assets', 'self-check'];
...
await writeFile(join(tempDir, 'render-stub.mjs'), 'export const SCREEN_WIDTH = 375; export const SCREEN_HEIGHT = 667;');
...
const { default: TableLayout, CARD_ASPECT_RATIO } = await import(pathToFileURL(join(tempDir, 'layout.mjs')));
...
for (const frameName of ['tile_back_green_vertical', 'tile_red_fu_vertical', 'meld_red_horizontal_shang', 'tile_black_zi_vertical']) {
  if (!atlas.frames || !atlas.frames[frameName] || !atlas.frames[frameName].frame) {
    throw new Error(`missing atlas frame ${frameName}`);
  }
}
const first24CardMap = buildCardAtlasFrameMap(atlas);
if (!first24CardMap.shang || !first24CardMap.shang[0].rotateCw) {
  throw new Error('horizontal shang frame should be mapped from first 24 and rotated clockwise');
}
if (!first24CardMap.fu || first24CardMap.fu[0].rotateCw) {
  throw new Error('vertical fu frame should be mapped from first 24 without rotation');
}
if (Object.keys(first24CardMap).length !== DEFAULT_RULES.cardSymbols.length) {
  throw new Error('atlas should map all 24 configured card labels');
}
...
for (const [width, height] of [[568, 320], [667, 375], [844, 390], [932, 430], [320, 568]]) {
  ...
  if (layout.handCards.length) {
    ...
    const expectedStackStep = Math.max(1, Math.round(25 * (layout.cardHeight / 307)));
    if (layout.cardStep !== expectedStackStep) {
      throw new Error(`phrase stack offset should scale from 25px at ${width}x${height}`);
    }
    ...
    Object.values(groupedByPhrase).forEach((cards) => {
      ...
      for (let i = 1; i < sorted.length; i++) {
        if (Math.abs((sorted[i].y - sorted[i - 1].y) - expectedStackStep) > 1) {
          throw new Error(`same-phrase cards should stack with scaled 25px offset at ${width}x${height}`);
        }
      }
